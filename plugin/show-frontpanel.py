...
class Plugin(CliPlugin):
    ...
    def _print(self, state, arguments, output, **_kwargs):
        ...
        cmd = ['/usr/local/bin/frontpanel', '-image', chassis.type, '-image-protocol', protocol]
        proc = subprocess.run(cmd, stdout=sys.stdout, stderr=subprocess.PIPE, text=True)
        if proc.returncode != 0:
            output.print(f"Failed to render front panel image (protocol={protocol}): {proc.stderr.strip()}")
